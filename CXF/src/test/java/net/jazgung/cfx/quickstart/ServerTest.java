...
public class ServerTest extends GenericTest {
	private void invoke(Server server) {
		...
		QuickStartService service = (new ClientFactory<QuickStartService>(WebServiceUtils.calcWsdlhUrl(QuickStartService.class,
				QuickStartService.ADDRESS), QuickStartService.SERVICE_NAME)).getService(QuickStartService.PORT_NAME,
